...
class TestEvalEvalshaProperties < Minitest::Test
  ...
  # For any valid Lua script and parameter set, executing via eval and then via evalsha (after loading) should produce identical results
  ...
  def test_script_execution_consistency
    ...
    100.times do
      ...
      assert_equal eval_result, evalsha_result, 
        "eval and evalsha should produce identical results for script: #{script}, keys: #{keys}, args: #{args}"
    end
  end

  # **Feature: eval-evalsha-commands, Property 2: Parameter round-trip preservation**
  # Property 2: Parameter round-trip preservation  
  # For any keys array and args array, a Lua script that returns both KEYS and ARGV should receive the exact parameters that were passed in
  ...
  # For any string that is not a valid 40-character hexadecimal SHA1 hash, evalsha should raise an ArgumentError before attempting server communication
  ...
  # For any valid SHA1 hash format that doesn't exist in the server's script cache, evalsha should raise a CommandError indicating script not found
  ...
  # For any Lua script that returns values of different types (string, number, boolean, array, nil), the Ruby client should convert them to appropriate Ruby types
  ...
  def test_type_conversion_correctness
    ...
    100.times do
      ...
      test_cases = [
        ...
        ["return 0", Integer, ->(result) { result.is_a?(Integer) && result == 0 }],
        ["return 3.14", Float, ->(result) { result.is_a?(Float) && (result - 3.14).abs < 0.001 }],
        ["return -2.5", Float, ->(result) { result.is_a?(Float) && (result + 2.5).abs < 0.001 }],
        ["return true", TrueClass, ->(result) { result == true }],
        ["return false", FalseClass, ->(result) { result == false }],
        ["return nil", NilClass, ->(result) { result.nil? }],
        ["return {1, 2, 3}", Array, ->(result) { result.is_a?(Array) && result == [1, 2, 3] }],
        ["return {}", Array, ->(result) { result.is_a?(Array) && result == [] }],
        ["return {'a', 'b', 'c'}", Array, ->(result) { result.is_a?(Array) && result == ["a", "b", "c"] }],
        ["return {1, 'mixed', true, nil}", Array, ->(result) { result.is_a?(Array) && result == [1, "mixed", true, nil] }],
      ...
      assert validation.call(eval_result), 
        "eval result should be correctly converted: script=#{script}, result=#{eval_result.inspect}, expected_type=#{expected_type}"
      
      # Test with evalsha (load script first)
      sha = r.script_load(script)
      evalsha_result = r.evalsha(sha)
      assert validation.call(evalsha_result), 
        "evalsha result should be correctly converted: script=#{script}, result=#{evalsha_result.inspect}, expected_type=#{expected_type}"
  ...
  # For any Lua script with syntax or runtime errors, the client should raise CommandError exceptions that preserve the original server error messages
  ...
  def test_cached_script_execution
    ...
    100.times do
      ...
      assert_equal expected_result, result,
        "Cached script execution should produce same result as direct eval: script=#{script}, keys=#{keys}, args=#{args}"
