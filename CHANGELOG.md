...
* feat(ruby): Add `Valkey::Logger`, a public logging API backed by the GLIDE core logger, so application logs and the driver's own internal logs share one destination, format, and level filter. `Valkey::Logger.init(level:, file_name:)` configures the logger only if it is not configured yet, `set_logger_config` replaces an existing configuration, `log(level, identifier, message, error:)` emits a message, and `enabled?(level)` reports whether a level would be logged. Levels are the symbols `:error`, `:warn`, `:info`, `:debug`, `:trace`, and `:off`; passing `file_name` writes hourly-rotated files under `./glide-logs` (or `GLIDE_LOG_DIR`) instead of the console. Filtering happens in Ruby, before the FFI call. A failed configuration raises the new `Valkey::LoggerError`, while a message the native layer rejects is reported on stderr rather than raised, so logging can never break the code path that emitted it ([#315](https://github.com/valkey-io/valkey-glide-ruby/issues/315)).
